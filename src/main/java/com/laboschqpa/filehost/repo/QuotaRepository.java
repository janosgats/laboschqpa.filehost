...
public interface QuotaRepository extends JpaRepository<Quota, Long> {
    ...
    @Query(value = "select * " +
            "from (select owner_user_id as subjectId, :categoryEnumVal_USER as subjectCategoryVal, sum(stored_file.size) as usedBytes " +
            "      from stored_file " +
            ...
            "UNION ALL " +
            "select * " +
            "from (select owner_team_id as subjectId, :categoryEnumVal_TEAM as subjectCategoryVal, sum(stored_file.size) as usedBytes " +
            "      from stored_file " +
            "               join indexed_file on stored_file.id = indexed_file.id " +
            "      where status in (:statusEnumVal_ADDED_TO_DATABASE_INDEX, :statusEnumVal_UPLOADING, :statusEnumVal_UPLOADED, :statusEnumVal_AVAILABLE, :statusEnumVal_FAILED, :statusEnumVal_ABORTED_BY_FILE_HOST) " +
